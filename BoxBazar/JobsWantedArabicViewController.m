//
//  JobsWantedArabicViewController.m
//  BoxBazar
//
...
@interface JobsWantedArabicViewController ()<ApiRequestdelegate,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,UISearchDisplayDelegate,UISearchResultsUpdating>
{
...
@implementation JobsWantedArabicViewController
...
-(void)customtoggleview
{
  //  [self.navigationController.view showActivityViewWithLabel:@"Please Wait..."];
    ...
    NSString *strurl=[NSString stringWithFormat:@"%@%@/%@/%@/%@?category=%@",BaseUrl,strtoken,module,arabic,strCityId,@"5"];
...
-(void)responseOption5:(NSMutableDictionary *)responseDict
{
    ...
    NSString *strtoken=[[NSUserDefaults standardUserDefaults]objectForKey:@"Token"];
    NSString *strCityId=[[NSUserDefaults standardUserDefaults]objectForKey:@"CityId"];
    NSString *strurl=[NSString stringWithFormat:@"%@%@/%@/%@/%@?module=%@&submodule=%@&parent=%@",BaseUrl,strtoken,Categoty,arabic,strCityId,strnameUrlparameter,@"category",@"1"];
   // [requested SubCategoryRequest5:nil withUrl:strurl];
        ...
        dispatch_async (dispatch_get_main_queue(), ^{
            ...
            } else
            {
                if(data1 != nil) {
                    NSError *err;
                    NSMutableDictionary *responseJSON = [NSJSONSerialization JSONObjectWithData:data1 options:kNilOptions error:&err];
                    [self responseSubCategory5:responseJSON];
                    [DejalBezelActivityView removeView];
                }
            }
        });
...
-(void)responseSubCategory5:(NSMutableDictionary *)responseDict
{
    ...
    if ([strstatus isEqualToString:@"1"])
    {
        ...
        NSString *result = [[arr valueForKey:@"id"] componentsJoinedByString:@","];
        
        NSDictionary *dict=[[NSDictionary alloc]initWithObjectsAndKeys:@"0",@"haschaild",result,@"id",@"All Jobs Wanted",@"name", nil];
        ...
        theSearchBar.placeholder = @"Search Jobs Wanted";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tabl.tag==2)
    {
        ...
        if ([strname isEqualToString:@"All Jobs Wanted"])
        {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        {
            if ([strModuleName isEqualToString:@"All Jobs Wanted"])
            {
                [DejalBezelActivityView activityViewForView:self.view withLabel:@"please wait..."];
                NSString *struseridnum=[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults]objectForKey:@"userid"]];
                NSString *strtoken=[[NSUserDefaults standardUserDefaults]objectForKey:@"Token"];
                NSString *strCityId=[[NSUserDefaults standardUserDefaults]objectForKey:@"CityId"];
                NSString *post = [NSString stringWithFormat:@"module=%@&user_id=%@&category=%@",strModuleUrlParameter,struseridnum,strModuleId];
