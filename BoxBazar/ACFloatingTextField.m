...
- (CGRect)textRectForBounds:(CGRect)bounds {
    return CGRectMake(0, 4, bounds.size.width, bounds.size.height);
}
...
-(void)initialization{
    ...
    if (_btmLineColor==nil) {
        _btmLineColor = [UIColor lightGrayColor];
    }
    ...
    _labelPlaceholder = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width-5, CGRectGetHeight(self.frame))];
