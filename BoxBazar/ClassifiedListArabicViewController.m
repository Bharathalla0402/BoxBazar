//
//  ClassifiedListArabicViewController.m
//  BoxBazar
//
...
@interface ClassifiedListArabicViewController ()<ApiRequestdelegate,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,UISearchDisplayDelegate,UISearchResultsUpdating>
{
...
@implementation ClassifiedListArabicViewController
...
- (void)viewDidLoad {
  ...
  //  _arrChildCategory=[[NSMutableArray alloc]init];
...
-(void)customtoggleview
{
   // [self.navigationController.view showActivityViewWithLabel:@"Please Wait..."];
    ...
    NSString *strurl=[NSString stringWithFormat:@"%@%@/%@/%@/%@?category=%@",BaseUrl,strtoken,module,arabic,strCityId,@"2"];
        ...
        dispatch_async (dispatch_get_main_queue(), ^{
            ...
            } else
            {
                if(data1 != nil) {
                    ...
                    [self responseOption3:responseJSON];
        ...
        dispatch_async (dispatch_get_main_queue(), ^{
            ...
            } else
            {
                if(data1 != nil) {
                    NSError *err;
                    NSMutableDictionary *responseJSON = [NSJSONSerialization JSONObjectWithData:data1 options:kNilOptions error:&err];
                    [self responseSubCategory3:responseJSON];
                    [DejalBezelActivityView removeView];
                }
            }
        });
