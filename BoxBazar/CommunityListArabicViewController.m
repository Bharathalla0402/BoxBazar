//
//  CommunityListArabicViewController.m
//  BoxBazar
//
...
@interface CommunityListArabicViewController ()<ApiRequestdelegate,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,UISearchDisplayDelegate,UISearchResultsUpdating>
{
...
@implementation CommunityListArabicViewController
...
- (void)viewDidLoad {
    ...
    NSDictionary *dict=[[NSDictionary alloc]initWithObjectsAndKeys:@"0",@"haschaild",@"0",@"id",@"All Community",@"name", nil];
...
-(void)customtoggleview
{
  //  [self.navigationController.view showActivityViewWithLabel:@"Please Wait..."];
    ...
    NSString *strurl=[NSString stringWithFormat:@"%@%@/%@/%@/%@?category=%@",BaseUrl,strtoken,module,arabic,strCityId,@"6"];
  //  [requested OptionRequest8:nil withUrl:strurl];
        ...
        dispatch_async (dispatch_get_main_queue(), ^{
            ...
            } else
            {
                if(data1 != nil) {
                    ...
                    [self responseOption8:responseJSON];
...
-(void)responseOption8:(NSMutableDictionary *)responseDict
{
  ...
  //  [requested SubCategoryRequest8:nil withUrl:strurl];
        ...
        dispatch_async (dispatch_get_main_queue(), ^{
            ...
            } else
            {
                if(data1 != nil) {
                    NSError *err;
                    NSMutableDictionary *responseJSON = [NSJSONSerialization JSONObjectWithData:data1 options:kNilOptions error:&err];
                    [self responseSubCategory8:responseJSON];
                    [DejalBezelActivityView removeView];
                }
            }
        });
...
-(void)responseSubCategory8:(NSMutableDictionary *)responseDict
{
    ...
    if ([strstatus isEqualToString:@"1"])
    {
        ...
        theSearchBar.placeholder = @"Search Community";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tabl.tag==2)
    {
        ...
        if ([strname isEqualToString:@"All Community"])
        {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        {
            if ([strModuleName isEqualToString:@"All Community"])
            {
                [DejalBezelActivityView activityViewForView:self.view withLabel:@"please wait..."];
