//
//  PropertySaleListArabicViewController.m
//  BoxBazar
//
...
@interface PropertySaleListArabicViewController ()<ApiRequestdelegate,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,UISearchDisplayDelegate,UISearchResultsUpdating>
{
...
@implementation PropertySaleListArabicViewController
...
- (void)viewDidLoad {
    ...
    NSDictionary *dict=[[NSDictionary alloc]initWithObjectsAndKeys:@"0",@"haschaild",@"0",@"id",@"All Property For Sale",@"name", nil];
...
-(void)customtoggleview
{
  //  [self.navigationController.view showActivityViewWithLabel:@"Please Wait..."];
    ...
    NSString *strurl=[NSString stringWithFormat:@"%@%@/%@/%@/%@?category=%@",BaseUrl,strtoken,module,arabic,strCityId,@"3"];
        ...
        dispatch_async (dispatch_get_main_queue(), ^{
            ...
            } else
            {
                if(data1 != nil) {
                    ...
                    [self responseOption7:responseJSON];
...
-(void)responseOption7:(NSMutableDictionary *)responseDict
{
 ...
 //   [requested SubCategoryRequest7:nil withUrl:strurl];
        ...
        dispatch_async (dispatch_get_main_queue(), ^{
            ...
            } else
            {
                if(data1 != nil) {
                    NSError *err;
                    NSMutableDictionary *responseJSON = [NSJSONSerialization JSONObjectWithData:data1 options:kNilOptions error:&err];
                    [self responseSubCategory7:responseJSON];
                    [DejalBezelActivityView removeView];
                }
            }
        });
...
-(void)responseSubCategory7:(NSMutableDictionary *)responseDict
{
    ...
    if ([strstatus isEqualToString:@"1"])
    {
        ...
        theSearchBar.placeholder = @"Search Property on Sale";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tabl.tag==2)
    {
        ...
        if ([strname isEqualToString:@"All Property For Sale"])
        {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        {
            if ([strModuleName isEqualToString:@"All Property For Sale"])
            {
                [DejalBezelActivityView activityViewForView:self.view withLabel:@"please wait..."];
